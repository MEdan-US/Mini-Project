...
* **Ý nghĩa:** Việc tăng thời gian dùng AI không hề giúp sinh viên tăng GPA một cách tuyến tính. Hiệu quả học tập thực chất có thể phụ thuộc vào *cách thức sử dụng* (như kỹ năng prompt, mục đích sử dụng) chứ không nằm ở *thời lượng sử dụng*.
...
- **Gradient Boosting Classifier**
  - Trên tập test, mô hình cho chỉ số `F1-Score` cao nhất với 0.5326 và các chỉ số còn lại trừ `Precision` đều cao nhất. Bên cạnh đó, độ chênh lệch giữa tập train và tập test rất thấp, chỉ khoảng 0.01 - 0.014 => mô hình không bị hiện tượng overfitting và có khả năng tổng quát hóa tốt nhất.
- **Random Forest**
  - Các chỉ số trên tập test đều ổn định trong khoảng 0.50 - 0.52, riêng chỉ số `Test Precision` đạt cao nhất nhóm với 0.5659. Điều này cho thấy khi mô hình dự đoán một người ở một mức độ kiệt sức cụ thể, mức độ tin cậy của quyết định đó là cao nhất.
  - Giữa tập train và test có sự chênh lệch nhẹ khoảng 0.03 - 0.04 => mô hình kiểm soát dữ liệu học tập tốt hơn một chút nhưng nhìn chung không bị overfitting đáng kể.
- **Logistic Regression**:
  - Logistic Regression có kết quả thấp nhất trong ba mô hình, nhưng vẫn là một baseline tốt vì khoảng cách hiệu suất so với Random Forest là rất nhỏ.
  - `Train F1` là 0.5205 và `Test F1` là 0.5179, hai giá trị rất gần nhau (chênh lệch dưới 0.003). Điều này cho thấy mô hình cực kỳ ổn định và hầu như không bị overfitting.
  - Mô hình có `Test Recall` (0.5055) thấp hơn `Test Precision` (0.5487), nghĩa là mô hình có xu hướng đưa ra các dự đoán mang tính an toàn cao, ít bị đoán sai một người không kiệt sức thành kiệt sức, nhưng lại chấp nhận bỏ sót một lượng khá lớn các trường hợp thực tế thuộc các mức độ kiệt sức khác nhau.
  - Vì Logistic Regression là mô hình tuyến tính, nó có thể chưa mô tả tốt các quan hệ phi tuyến phức tạp giữa các đặc trưng đầu vào trong bài toán phân loại đa lớp này.
